...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        operands = exp.rest.map(calc_eval) # UPDATE THIS FOR Q2
...
def floor_div(args):
    ...
    """
    # BEGIN SOLUTION Q2
    assert args.__len__() >= 2,  'at least two arguments'
    first = args.first // args.rest.first
    if args.rest.rest is nil:
        return first
    else:
        return floor_div(Pair(first, args.rest.rest))
...
def eval_and(expressions):
    ...
    """
    # BEGIN SOLUTION Q3  
    # in Scheme only #f is a False value!
    if expressions == nil:
        return True
    if expressions.rest == nil:  # 只有一个数   (and 1) --> 1
        return expressions.first
    if expressions.rest.rest is nil:
        if expressions.first is scheme_f: # 0 == False >>> True     False == 0  >>> True
            return False
        else:
