...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def check(sentence):
        list = remove_punctuation(sentence).split()
        for word in list:
            for key in subject:
                if word.lower() == key:
                    return True 
...
def accuracy(typed, source):
    ...
    i = 0
    correct = 0
    while i < min(source_len, typed_len):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if typed_word == word:
            return typed_word
        elif diff_function(typed_word, word, limit) < record:
            record = diff_function(typed_word, word, limit)
            answer = word 
    return answer
    # END PROBLEM 5
...
def fastest_words(match):
    ...
    for word in word_indices:
        record = time(match, 0, word)
        winner = 0
        for player in player_indices:
            if time(match, player, word) < record:
                winner = player
                record = time(match, player, word)
        list[winner] += [get_word(match, word)]
    return list
