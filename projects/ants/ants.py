    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.is_container:
                if place.ant.is_container == False:
    ...
    def double(self):
        ...
        if self.is_container:
            if not self.doubled:
                self.damage *= 2
                self.doubled = True
            if self.ant_contained != None and not self.ant_contained.doubled:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while bee_place.is_hive == False:# [] == None -- False
            if bee_place.bees and self.lower_bound <= distance <= self.upper_bound:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            if self.place.bees:
                bee_to_die = random_bee(self.place.bees)
                bee_to_die.reduce_health(bee_to_die.health)
